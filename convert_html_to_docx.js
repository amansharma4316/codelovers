...
function generateDocx(doc) {
    ...
    this.createHiperlinkNode = function(node, xmlDoc, numLink) {
        ...
        for (var j = 0; j < node.childNodes.length; j++) {
            if (node.childNodes[j].nodeName === 'IMG') {
                ...
                if (node.parentNode.nodeName === 'BODY') { var p = xmlDoc.createElement("w:p");
                    p.appendChild(nR); return p; } else return nR;
            }
        }
    ...
    this.createMyTextNode = function(node, xmlDoc) {
        ...
        if (orientationRTL) {
            ...
            var rtl = xmlDoc.createElement("w:rtl");
        ...
        // CREAR NODO LISTA . estos tags directamente se insertan en el body independientemente, en parrafos diferenciados
            ...
            if (granFather) {
                ...
                while (abu && granFather.nodeName === 'LI') {
                    ...
                    if (granFather.parentNode.parentNode) { abu = true;
                        granFather = granFather.parentNode.parentNode; } else abu = false;
                }
            }
